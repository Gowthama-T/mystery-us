    ...
    >
      <Card className="glass-card border-0 p-6 h-full relative overflow-visible">
        ...
        <div className="relative z-10 flex flex-col items-center text-center">
          <motion.div whileHover={{ scale: 1.1 }} className={`mb-4 relative`}>
            <Avatar className={`w-20 h-20 border-2 border-${color}/50`}>
              <AvatarFallback
                className={`bg-${color}/20 text-${color} font-display text-lg`}
              >
          ...
          <Badge
            variant="outline"
            className={`mb-2 border-${color}/30 text-${color}/90 text-xs`}
          >
            {person.role}
          </Badge>
...
export function Coordinators() {
  ...
  const studentCoordinators =
    coordinators?.filter((c) => c.type === "student") || [];
  const techCoordinators =
    coordinators?.filter((c) => c.type === "tech") || [];
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12">
            <p className="text-destructive">
              Failed to load coordinators. Please try again later.
            </p>
          </div>
        ) : (
          <div className="space-y-16">
