...
import { ChevronDown, Trophy, Users, AlertCircle, Target, Circle, Hand, Dribbble, Dumbbell } from "lucide-react";
      ...
      >
        <div className="p-6">
          <div className="flex items-center justify-between gap-4 flex-wrap">
            <div className="flex items-center gap-4">
              <motion.div whileHover={{ rotate: 10, scale: 1.1 }} className="w-14 h-14 rounded-xl glass flex items-center justify-center">
                <IconComponent className={`w-7 h-7 text-${sport.color}`} />
              </motion.div>
              <div>
            ...
            <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.3 }} className={`text-${sport.color}`}>
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {sport.highlights.map((highlight) => (
              <Badge key={highlight} variant="outline" className={`border-${sport.color}/30 text-${sport.color}/90 text-xs`}>
                {highlight}
              </Badge>
            ))}
          </div>
...
export function SportsRules() {
  ...
  const { data: sports, isLoading, error } = useQuery<Sport[]>({
    queryKey: ["https://mystery-us.onrender.com/api/sports"],
  });

  return (
    <section id="sports" ref={ref} className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden">
